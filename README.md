...
| `readOnly`                                                                | `boolean`                                | `true`         | With `readOnly={false}`, users can type a date using ASCII or Devanagari digits; it is committed on Enter or blur, and invalid text reverts. |
...
With `readOnly={false}`, typed text using ASCII or Devanagari digits is committed on Enter or blur — a normal submit-button
